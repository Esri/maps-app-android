...
public class PopupFragment extends Fragment {
	...
	/*
	 * factory method to pass MapView into PopupFragment
	 */
	public static PopupFragment newInstance(MapView mapView){
		// create a new PopupFragment
		PopupFragment popup = new PopupFragment();
		// get the MapView
		popup.mMapView = mapView;
		return popup;
	}
				...
				if (popup != null) {
					if (popup.isEditMode()) {
						if (item.getItemId() == R.id.menu_save
								|| item.getItemId() == R.id.menu_camera) {
						...
						if (((item.getItemId() == R.id.menu_edit) && (popup
								.isEditable()))
								|| ((item.getItemId() == R.id.menu_delete) && (popup
										.isDeletable()))) {
							item.setVisible(true);
							item.setEnabled(true);
						} else {
		...
		if (popupInfo instanceof ArcGISPopupInfo) {
			...
			for (Layer layer : layers) {
				if ((layer instanceof ArcGISFeatureLayer)
						&& (layer.getUrl().compareToIgnoreCase(
								agsPopupInfo.getLayerUrl()) == 0)) {
