				...
				if (webmap != null) {
					getActivity().runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							// the MapView.
							mCompass = new Compass(mapView.getContext(), null,
									mapView);
							mapView.addView(mCompass);
							mapView.zoomin();
							setMapView(mapView);
		...
		locDispMgr.setLocationListener(new LocationListener() {
			...
			public void onLocationChanged(Location loc) {
				...
				if (!locationChanged) {
					...
					locDispMgr.setAutoPanMode(AutoPanMode.COMPASS);
				}
			}
	...
	public class ReverseGeocodingAsyncTask extends
		...
		protected void onPostExecute(LocatorReverseGeocodeResult result) {
			...
			if (result != null && result.getAddressFields() != null) {
				...
				// Toast.makeText(MapsAppActivity.this,
				// mLocationLayerPointString,
				// Toast.LENGTH_LONG).show();

				// create text symbol for result address
				TextSymbol textSymbol = new TextSymbol(20, address.toString(),
						Color.BLACK);
...
>>>>>>> 077e288e3417f6b0541d23030bba87d21150e96d
}
